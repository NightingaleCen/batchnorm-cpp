...
std::vector<torch::Tensor> batchnorm1d_forward(
  torch::Tensor input,
  torch::Tensor gamma,
  torch::Tensor beta) {
  CHECK_INPUT(input);
  ...
  return batchnorm1d_cuda_forward(input, gamma, beta);
}

std::vector<torch::Tensor> batchnorm1d_backward(
  torch::Tensor d_output,
  torch::Tensor input,
  torch::Tensor mu,
  torch::Tensor sigma2,
  ...
  return batchnorm1d_cuda_backward(
    d_output,
    input,
    mu,
    sigma2,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("batchnorm1d_forward", &batchnorm1d_forward, "batchnorm1d forward (CUDA)");
  m.def("batchnorm1d_backward", &batchnorm1d_backward, "batchnorm1d backward (CUDA)");
